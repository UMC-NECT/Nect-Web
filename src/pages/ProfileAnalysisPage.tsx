			...
			? [
					{
						...
						description: body.roleRecommendation.leader ?? '',
					},
					{
						role: '팀원',
						title: '현재 모집중인 프로젝트를 추천할게요!',
						description: body.roleRecommendation.team_member ?? '',
					},
				]
			: []
	const growGuide =
	...
	return (
		<div className='flex flex-col justify-center pt-32'>
			...
			<div className='bg-bg-gray w-full rounded-100 mt-16 pt-20 pb-24 px-12 shadow-inner-neutral-1'>
				<div className='flex flex-col items-center px-60 mb-[112px]'>
					...
					<ContentSection title='협업 스타일' className='mb-20'>
						<ProfileRadarChart
							data={radarData}
							className='w-[600px] h-[500px] mx-auto bg-neutral-000 rounded-[20px]'
						/>
					</ContentSection>
			...
			<div className='flex gap-5 min-w-[660px] justify-center mx-auto mt-[112px] mb-24'>
				<Button color='secondary' size='xl' fullWidth onClick={() => navigate('/')}>
					메인홈으로 이동
				</Button>
				<Button color='primary' onClick={() => navigate('/idea-analyze')} size='xl' fullWidth>
					아이디어 분석 후 프로젝트 생성
				</Button>
			</div>
		</div>

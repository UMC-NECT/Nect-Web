...
const IdeaAnalyzePage = () => {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			setSubmitError(err instanceof Error ? err.message : '분석 요청에 실패했습니다. 다시 시도해 주세요.')
		} finally {
				...
				<div className='relative bg-bg-gray rounded-[100px] z-10 w-screen -ml-[calc((100vw-100%)/2)] -mr-[calc((100vw-100%)/2)] min-h-screen py-8 mt-[52px]'>
					...
					<div className='mt-[52px] w-[940px] mx-auto'>
						...
						<div className='flex flex-col items-center justify-center mt-16 mb-8'>
							{submitError && (
								<p className='text-semantic-600 text-[14px] mb-3' role='alert'>
									{submitError}
								</p>
							)}
							<button
							...
							>
								{isSubmitting ? '분석 중...' : 'AI 아이디어 분석하기'}
							</button>
						</div>
					</div>
				</div>

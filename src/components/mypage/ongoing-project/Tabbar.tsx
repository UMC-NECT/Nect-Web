	...
	return (
		<div className='flex items-center mb-7'>
				...
				<span
					className={`title-3 font-semibold text-center ${
						currentTab === '프로젝트 설정'
							...
							: 'text-neutral-400 hover:text-neutral-700 duration-150 ease-in-out'
					}`}
				>
					프로젝트 설정
				</span>
				<div className={`h-0.75 w-full ${currentTab === '프로젝트 설정' ? 'bg-primary-400-normal' : 'bg-neutral-300'}`} />
			</button>
			...
			<button type='button' onClick={() => onClick('팀원 관리')} className='flex flex-col gap-3 w-30 pt-2.5'>
				...
				<div className={`h-0.75 w-full ${currentTab === '팀원 관리' ? 'bg-primary-400-normal' : 'bg-neutral-300'}`} />
			</button>
		</div>
	)
}

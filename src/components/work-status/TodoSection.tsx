...
const TodoSection = ({ status, children }: TodoSectionProps) => {
	...
	return (
		<div className={cn('flex flex-col gap-2 p-2', colorConfig.bgColor, 'rounded-16 w-full')}>
			{children}
			<button className='bg-[rgba(250,250,250,0.2)] border border-neutral-200 rounded-12 px-[12px] py-[10px] flex items-center gap-[6px] relative shadow-inner-neutral-2' onClick={() => openMissionModal(undefined, undefined, undefined, true, false, status)}>

...
const WorkspaceHeader = ({ onNavigate }: WorkspaceHeaderProps) => {
	...
	const exploreMenuItems = [{ name: '프로젝트 찾기' }, { name: '팀원 찾기' }, { name: '출시 프로젝트' }]
		...
		<header className='fixed top-0 left-0 right-0 bg-white z-50 shadow-[0px_4px_20px_0px_rgba(25,25,25,0.02)]'>
			...
			<div className='fixed left-[92px] right-[92px] h-[66px] '>
				<div className='mx-auto flex h-full items-center gap-9 px-6 relative'>
						...
						<div className='relative'>
							<button
								...
								className={`text-[18px] font-medium transition-colors ${
									showExploreMenu ? 'text-primary-500-normal' : 'text-neutral-400'
								}`}
							>
								프로젝트ㆍ팀원 탐색
							</button>
							...
							{showExploreMenu && (
								...
								>
									{exploreMenuItems.map((item, index) => (
										<div key={item.name}>
											<button className='w-full h-[54px] px-4 text-left text-[16px] font-medium text-neutral-900 hover:bg-neutral-50 transition-colors flex items-center'>
												{item.name}
											</button>
							...
							<button
								...
								onClick={() => {
									...
									if (projects.length > 0 && projects[0]) {
										...
										setProjectId(targetProjectId, userId)
								...
								className={`text-[18px] font-medium transition-colors ${
									showWorkspaceMenu ? 'text-primary-500-normal' : 'text-neutral-900 hover:text-neutral-900'
								}`}
							>
								팀 작업실
							</button>
					...
					{/* 검색 아이콘 */}
					<button className='flex h-10 w-10 items-center justify-center' aria-label='검색'>

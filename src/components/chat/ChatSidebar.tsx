...
interface ChatSidebarProps {
	unreadCount?: number
	selectedView?: 'message' | 'cloud'
	onMessageClick?: () => void
	onCloudClick?: () => void
	onSettingsClick?: () => void
}

const ChatSidebar = ({ unreadCount = 0, selectedView = 'message', onMessageClick, onCloudClick, onSettingsClick }: ChatSidebarProps) => {
				...
				<div className="flex flex-col gap-[20px] items-start">
					...
					<MessageIcon
						className={`w-[30px] h-[30px] ${
							selectedView === 'message' ? 'text-neutral-000' : 'text-status-info-cool-gray-neutral'
						}`}
					/>
						{unreadCount > 0 && (
					...
					<CloudIcon
						className={`w-[30px] h-[30px] ${
							selectedView === 'cloud' ? 'text-neutral-000' : 'text-status-info-cool-gray-neutral'
						}`}
					/>
					</button>
				</div>

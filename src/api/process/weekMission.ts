...
export const getWeekMission = async (projectId: string, weeks: string): Promise<ResponseMissionDto> => {
    const { data } = await api.get(`/api/v1/projects/${projectId}/week-missions/week`, { params: { weeks: weeks } })
    return data
}
